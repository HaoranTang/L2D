...
class CameraManager(object):
    def __init__(self, parent_actor, hud, gamma_correction):
        ...
        for item in self.sensors:
            ...
            if item[0].startswith('sensor.camera'):
                ...
                #     cam_bp.set_attribute("image_size_y",str(hud.dim[1]))
                #     cam_location = carla.Location(2,0,1.4)
                #     cam_rotation = carla.Rotation(0,0,0)
                #     cam_transform = carla.Transform(cam_location,cam_rotation)
                #     ego_cam = world.spawn_actor(cam_bp,cam_transform,attach_to=self._parent, attachment_type=carla.AttachmentType.Rigid)
                #     ego_cam.listen(lambda image: image.save_to_disk('./out/%.6d.png' % image.frame))
    ...
    def _parse_image(weak_self, image):
        ...
        if self.sensors[self.index][0].startswith('sensor.lidar'):
            ...
            array = array[:, :, ::-1]
            self.surface = pygame.surfarray.make_surface(array.swapaxes(0, 1))
        if self.recording and int(self.hud.simulation_time) % 5 == 0:
