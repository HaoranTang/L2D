        ...
        with TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name) as writer:
            ...
            for step in range(total_timesteps):
                ...
                if self.verbose >= 1 and done and log_interval is not None and len(episode_rewards) % log_interval == 0:
                    ...
                    Logger.record("mean 100 episode reward", mean_reward)
                    Logger.record('ep_rewmean', safe_mean([ep_info['r'] for ep_info in ep_info_buf]))
                    Logger.record('eplenmean', safe_mean([ep_info['l'] for ep_info in ep_info_buf]))
                    Logger.record("n_updates", self.n_updates)
                    Logger.record("current_lr", current_lr)
                    Logger.record("fps", fps)
                    Logger.record('time_elapsed', "{:.2f}".format(time.time() - start_time))
                    if len(infos_values) > 0:
                        for (name, val) in zip(self.infos_names, infos_values):
                            Logger.record(name, val)
                    Logger.record("total timesteps", step)
                    Logger.dump()
                    # Reset infos:
                    infos_values = []
            if is_teleop_env:
