...
class Env(gym.Env):
    ...
    def cnn_observation_space(self):
        return gym.spaces.Box(low=0.0, high=255.0, shape=(3,80,160), dtype=np.uint8)
    ...
    def step(self, action):
        ...
        im.save_to_disk('buffer_ddpg_mlp/%08d' % im.frame)
        im_third.save_to_disk('buffer_ddpg_mlp/%08d' % im_third.frame)
...
class NewEnv(gym.Env):
    ...
    def cnn_observation_space(self):
        return spaces.Box(low=0.0, high=255.0, shape=(80, 160, 3), dtype=np.uint8)
    ...
    def step(self, action):
        ...
        # comment out when training
        im.save_to_disk('buffer_sac_cnn/%08d' % im.frame)
        im_third.save_to_disk('buffer_sac_cnn/%08d' % im_third.frame)
        ...
        return self.postprocessing_step(action, im, reward, done, {})
